...
public class FarkleScoring {
	 ...
	 */
	public Collection<DiceScore> computeDiceScores(Dice dice) {
		final Collection<DiceScore> result = new ArrayList<>();
		while (dice.getDieCount() > 0) {
			DiceScore best = null;
			for (final DiceScorer diceScorer : diceScorers) {
				final DiceScore score = diceScorer.getScore(dice);
