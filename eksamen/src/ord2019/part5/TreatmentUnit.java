...
public class TreatmentUnit {
	...
	private final Collection<TreatmentListener> treatmentListeners = new ArrayList<>();

	public void addTreatmentListener(final TreatmentListener listener) {
		treatmentListeners.add(listener);
	}

	public void removeTreatmentListener(final TreatmentListener listener) {
		treatmentListeners.remove(listener);
	}

	private void fireTreatmentStarted(final Doctor doctor, final Patient patient) {
		for (final TreatmentListener treatmentListener : treatmentListeners) {
			treatmentListener.treatmentStarted(doctor, patient, this);
		}
	}

	private void fireTreatmentFinished(final Doctor doctor, final Patient patient) {
		for (final TreatmentListener treatmentListener : treatmentListeners) {
			treatmentListener.treatmentFinished(doctor, patient, this);
		}
	}
	...
	public void treatmentFinished(final Doctor doctor) {
		...
		if (patient.requiresTreatment()) {
			...
			fireTreatmentFinished(doctor, patient);
		}
		startTreatment(doctor);
	}
}
