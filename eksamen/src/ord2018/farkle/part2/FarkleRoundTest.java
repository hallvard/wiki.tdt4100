...
public class FarkleRoundTest {
	...
	public void testDiceRound1() {
		...
		DiceImpl1Test.checkDieValues(round.getCurrentDice(), 1, 1, 5, 2, 2);

		nextValues = Arrays.asList(1, 2).iterator();
		round.keepAndRoll(new DiceImpl1(Arrays.asList(1, 1, 5), -1));
		checkDiceScores(round, 100, 100, 50);
		Assert.assertFalse(round.isFinished());
		DiceImpl1Test.checkDieValues(round.getCurrentDice(), 1, 2);
