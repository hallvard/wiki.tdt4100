...
public class DiceImpl2Test {

	static Dice dice(final Integer... values) {
		return new DiceImpl2(Arrays.asList(values), -1);
	}
	...
	public void testValueOf() {
		final Dice dice1 = DiceImpl2.valueOf("[1, 1, 2]");
		checkDieValues(dice1, 1, 1, 2);
		Assert.assertEquals(-1, dice1.getScore());

		final Dice dice2 = DiceImpl2.valueOf("[1, 1, 2] = 200");
		checkDieValues(dice2, 1, 1, 2);
		Assert.assertEquals(200, dice2.getScore());

		try {
			DiceImpl2.valueOf("1, 5, 2 = 200");
			Assert.fail();
		} catch (final Exception e) {
			Assert.assertTrue(e instanceof IllegalArgumentException);
		}
		try {
			DiceImpl2.valueOf("1, x, 2 = 200");
			Assert.fail();
		} catch (final Exception e) {
			Assert.assertTrue(e instanceof IllegalArgumentException);
		}
		try {
			DiceImpl2.valueOf("1, 5, 2 = x");
			Assert.fail();
		} catch (final Exception e) {
			Assert.assertTrue(e instanceof IllegalArgumentException);
		}
